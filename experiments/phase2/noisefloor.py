...
def main(argv=None) -> int:
    ...
    if failures:
        ...
        print()
        print("`centered_ranks` gives exactly tied members the same weight since the midrank")
        print("change, so an exact tie is no longer a source of disagreement. A pair one ulp")
        print("apart still gets distinct ranks and can still swap, and that is most of the")
        print("count above: the `close` column is what to read, not `ties`.")
        print()
        ...
        print("  - Continuous shaping measures 25x amplification against the rank")
        print("    transform's 2.65e5, so it does address near-ties. It is a different")
        print("    optimizer, not a free improvement: rank shaping is what buys robustness")
        print("    to outliers and to reward scale.")
