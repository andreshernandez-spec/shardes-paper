...
def main(argv=None) -> int:
    ...
    if args.latex:
        ...
        (gen / "tb6.tex").write_text("\n".join([
            ...
            "\\caption{Alignment $\\cos(\\Delta\\theta, -\\nabla L)$ of one ES update with the",
            "  gradient-descent direction of the next-token loss ($\\times 10^{-4}$, median over",
            "  5 perturbation seeds), at equal population; \\emph{1 unp.} is rank 1 without",
            "  mirrored pairs.",
        ...
        (gen / "tb6a.tex").write_text("\n".join([
            ...
            "\\caption{The alignment measurements, $\\cos(\\Delta\\theta, -\\nabla L)$, against both predictions",
