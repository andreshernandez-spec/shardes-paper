#!/usr/bin/env python
"""E15 analysis: real-model update--gradient alignment against F5's fits.

    python analysis_e15.py                     # reads results-e15/
    python analysis_e15.py --results DIR       # e.g. a partial fetch

Reads only committed artifacts: E1's update--gradient-alignment results
(experiments/phase0/results) and E15's per-cell JSONs. No GPU, no network.

The bridge being tested: F5 fitted log cos = a + b log(N/d_samp) on synthetic
objectives; E15 measured the same cosine on Qwen2.5-0.5B's teacher-forced NLL.
...
ARMS = {  # E15 strategy -> (E1 strategy or None, rank for sampling dimension)
...
def main(argv=None) -> int:
    ...
    for shaping in ("centered_ranks", "none"):
        ...
        for (strategy, n), d in sorted(cells.items()):
            ...
            med = d["cosine_median"]
            line = (f"  {strategy:13s} N={n:<4d} d_samp {d_eff:>11,}  "
    ...
    if args.latex:
        ...
        out.write_text("\n".join([
            ...
            "  centered-rank ES update and the true teacher-forced-NLL gradient",
            "  on Qwen2.5-0.5B",
            "  ($\\times 10^{-4}$; median [min, max] over 5 seeds), against the",
            "  synthetic alignment study's fitted prediction at the same",
            "  $N/d_{\\mathrm{samp}}$. The alignment",
