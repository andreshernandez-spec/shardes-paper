...
- **The memory column is a finding, not a gap.** Rank 1 on the real model
  does not fit one 16 GB chip even at N=32 (both placements OOM at D=1,
  recorded), and N=240 fits nowhere below D=8; the seed arm runs at every
  cell. The storage-for-compute trade the paper prices on the block holds
  at Qwen scale, in the same direction.
