...
def test_a_resumed_sweep_is_not_reported_as_dirty(tmp_path, monkeypatch):
    ...
    # env.json is tracked and rewritten every run, and figures/ is regenerated by plot.py.
    # Both are outputs like results/, so neither makes the code that produced them unreprod-
    # ucible. Missed on the first cut of this fix, and it cost a relaunch.
    (run.HERE / "env.json").write_text("{}")
    (run.HERE / "figures").mkdir(exist_ok=True)
    (run.HERE / "figures" / "f5.png").write_bytes(b"x")
    assert run.worktree_is_dirty() is False, "the sweep's own env.json/figures flagged it"
