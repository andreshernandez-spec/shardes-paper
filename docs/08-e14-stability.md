...
Bold = published checkmarked value (Qiu Table 4); multipliers are applied to each
algorithm's own published setting, which is the only fair definition of
perturbation. ES arm is mirrored rank 1 (the paper's headline strategy). 21 new
runs.
...
multiplier; the kl_beta=0 arm training fine. Any of these gets reported as found;
...
~2 h (~$2.8 each, 9 runs). Estimate $30-50 total, one rented A100, sequential,
