...
\begin{itemize}
  \item \textbf{Eight devices, one host, a fat fabric.} The GPU results are
    ...
    is the easy case for B. Section~\ref{subsec:boundary} tests one host boundary and confirms the
    concern: over a $9.2$\,GiB/s socket fabric the crossover moves
    substantially and the dense (seed-regenerated) arm flips from B to A.
    That boundary is socket, not InfiniBand, and a single bandwidth point;
    whether the flip persists at the faster InfiniBand a production cluster
    provides is not established, and the frozen model mispredicted this
    cell.
