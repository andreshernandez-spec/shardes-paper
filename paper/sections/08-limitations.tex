...
\begin{itemize}
  ...
  \item \textbf{The cost model has an open term on the low-rank side.}
    ...
    the low-rank side. In three cells B is slower than its local contraction
    plus the isolated all-reduce accounts for, by 0.6--0.7\,ms, so a
    collective inside a full generation costs more than the same collective
    in an empty program and the excess is not derived here. Attribution
    within a generation is also uneven: the regeneration-versus-evaluation
    split is measured on the A100 and pending on the v5e, and no achieved
    bandwidth is reported for the collective at sub-payload sizes.
