...
weight matrix and make the update cheap to reconstruct. Neither paper compares these
two update placements in a common implementation.

We built \textbf{shardes} \citep{shardes}, a JAX \citep{jax2018} library, to make that
...
\begin{itemize}
  ...
  \item \textbf{Does the result carry over?} We test a real language model, a slow
    connection between hosts, and both GPU and TPU hardware. We also measure the
    relative cost of low-rank perturbations on each platform.
  \item \textbf{Do cheaper perturbations remain useful?} We compare one-update alignment
    with a loss-reducing gradient direction and then train on Countdown. The tested
    ranks reach similar observed final accuracy, but differ early in training.
\end{itemize}
