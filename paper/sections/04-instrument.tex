...
important test in the instrument's repository. The guarantee is deliberately narrower
...
near-ties in generation (Section~\ref{sec:setup} states the measured
boundary).
...
Section~\ref{sec:results} shows what honoring that invariant buys: on the
16\,GB chip the dense baseline runs out of memory over most of the
measured grid, the low-rank family extends the feasible region well past
it (failing only at the largest cells), and the seed-regenerated family
covers every cell. The honest cost of this generality is
...
direct arithmetic. An existing model therefore does not run unmodified, and a
layer type the library has no seam for needs one written. That is the price
of never materializing the noise, paid at the API rather than at runtime.
...
under 10\,$\mu$s at every population the grids run and about 20\,$\mu$s at
$N{=}2^{18}$; the sort that
