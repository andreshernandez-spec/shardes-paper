...
training on Countdown on one A100. For the first two the fitness is the model's
...
same (model size, population, arm, datatype) configurations. A configuration that does
not fit a device is recorded as out of memory, not resized, because where each device runs out is part of
the comparison.
...
\paragraph{Provenance.} Every result record stamps its commit and environment,
configurations were committed before their runs, and every figure and table regenerates
from a script (Appendix~\ref{sec:repro}). The Countdown campaign was rerun once from a
clean tree and reproduced within seed noise; the curves here are the rerun's.
