...
moves $\theta$ by
\begin{equation}
  \Delta\theta = \frac{\alpha}{N\sigma}\sum_{i=1}^{N} w_i\,\epsilon_i ,
  \label{eq:update}
\end{equation}
where the weights $w_i$ come from the fitnesses \citep{wierstra2014,nesterov2017}. We use
centered ranks \citep{salimans2017}: each fitness is replaced by its rank, scaled to
$[-\tfrac12,\tfrac12]$, which makes the update ignore any monotone rescaling of the
reward. The sum in Eq.~\ref{eq:update} is the \emph{contraction} of the population
against its weights, and where it is computed is the subject of this paper.
...
rank $r$ & rank-$r$ factors, never materialized, in mirrored pairs \citep{eggroll2025} \\
...
\paragraph{Why one library.} A library whose central object is the population as a
dense (population, parameters) matrix, as evosax's is \citep{lange2022evosax}, has
nowhere to put a perturbation that is never materialized, and at language-model scale
that matrix does not fit. Comparing the two designs in their own codebases would mix the
design with the implementation. Section~\ref{sec:instrument} describes the library we
used instead.
